...
// Den rekursive hjelpemetoden for søking
	...
	public String BSTtilString(T bst) {
		String svar = bst.toString();

		return svar;
	}

	public int hoyde() {
		return hoyderekrusjon(rot);
	}
	
	//Oppgave a)
	public int hoyderekrusjon (BinaerTreNode<T> node) {
		
		if (node == null) {
			return -1;
		}
		
		int hoydevenstre = hoyderekrusjon(node.getVenstre());
		int hoydehoyre = hoyderekrusjon(node.getHoyre());
